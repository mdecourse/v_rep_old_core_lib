...
bool CStlFile::exportFunc(std::string fileName,bool poly,const std::vector<std::vector<float>*>& vertices,const std::vector<std::vector<int>*>& indices,const std::vector<std::string>& names)
{
    ...
    for (int i=0;i<int(vertices.size());i++)
        totalTriangles+=(int)indices[i]->size()/3;
...
{   
    ...
    for (int i=0;i<int(objects.size());i++)
    {
        ...
        if (it!=NULL)
        {
            ...
            totalTriangles+=(int)wind.size()/3;
        }
    }
    std::string headerText("Exported with V-REP");
    for (int i=0;i<80;i++)
...
bool CStlFile::_readFile(VArchive& archive,int format)
{
    ...
    { // ASCII STL
        ...
        unsigned int actualPosition=5;
        archive.readSingleLine(actualPosition,line,true);
        bool insideFacet=false;
        bool insideLoop=false;
        int vertexNb=0;
        C3Vector v[3];
        while (archive.readSingleLine(actualPosition,line,true))
        {
            if (tt::extractSpaceSeparatedWord(line,word))
            {
                ...
                if ((word=="vertex")&&insideLoop)
                {
                    ...
                    {
                        std::string v0,v1,v2;
                        if (tt::extractSpaceSeparatedWord(line,v0)&&tt::extractSpaceSeparatedWord(line,v1)&&tt::extractSpaceSeparatedWord(line,v2))
                        {
                            float w0,w1,w2;
                    ...
                    if (vertexNb==3)
                    {
                        indices->push_back(0+(int)vertices->size()/3);
                        indices->push_back(1+(int)vertices->size()/3);
                        indices->push_back(2+(int)vertices->size()/3);
                        for (int i=0;i<3;i++)
                        {
                            for (int j=0;j<3;j++)
