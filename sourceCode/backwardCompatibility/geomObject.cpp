...
{ // Routine needed for the transition to the new shape-Geom architecture (November 2012). This is only needed when loading files with serialization version 15 or lower
    ...
    if (_geomStruct->isCompound())
    {
        CGeomWrap* geomInfo=new CGeomWrap();
        geomInfo->setDynMaterialId_OLD(-2); // this triggers the creation of a material later!
    ...
    else
    {
        ...
        _geomStruct->insideColor.copyYourselfInto(&geomInfo->insideColor_DEPRECATED);
        _geomStruct->edgeColor.copyYourselfInto(&geomInfo->edgeColor_DEPRECATED);
        geomInfo->setVisibleEdges(_geomStruct->getVisibleEdges());
        geomInfo->setCulling(_geomStruct->getCulling());
        geomInfo->setInsideAndOutsideFacesSameColor_DEPRECATED(_geomStruct->getInsideAndOutsideFacesSameColor());
        geomInfo->setWireframe(_geomStruct->getWireframe());
        geomInfo->setEdgeWidth_DEPRECATED(_geomStruct->getEdgeWidth());
...
void CGeomObject_OLD::serialize(CSer& ar)
{
    ...
    {       // Loading
        ...
        // FOLLOWING CODE TO TRANSFORM (BRUTALLY) ALL COMPOUND SHAPES INTO SIMPLE SHAPES
        //if (_geomStruct->isCompound())
        //{
