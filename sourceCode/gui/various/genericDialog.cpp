                               ...
                               const char* initialText,const float titleColor[6],const float dialogColor[6])
{
    ...
    ogl::getMultilineTextInfo(titleText,lines,&maxWidthTitle,&maxHeight);
    lines.clear();
    ogl::getMultilineTextInfo(_editBoxText,lines,&maxWidth,&maxHeight);
    lines.clear();
    ...
    // Tweak:
    if (App::sc>1)
        maxWidth=int(float(maxWidth)*1.12f);
    ...
    horizContentSize+=2;
    horizContentSize=tt::getLimitedInt(12*App::sc,48*App::sc,horizContentSize);
    ...
    int hCellSize=14/App::sc;
    int vCellSize=14;
    CButtonBlock* b=new CButtonBlock(dialogWidth,dialogHeight,hCellSize,vCellSize,-1);
